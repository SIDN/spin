...
ip_store_t* ip_store_create() {
    ip_store_t* ip_store = (ip_store_t*) malloc(sizeof(ip_store_t));
...
static inline void split_key(unsigned char ip[16], uint64_t* k1, uint64_t* k2) {
    memcpy(k1, ip, 8);
    memcpy(k2, ip+8, 8);
}
...
void ip_store_add_ip(ip_store_t* ip_store, int ipv6, unsigned char ip[16]) {
    ...
    if (!ip_store_contains_ip(ip_store, ip)) {
        split_key(ip, &k1, &k2);
        el_new = (ip_store_el_t*)malloc(sizeof(ip_store_el_t));
        el_new->k1 = k1;
        el_new->k2 = k2;
        el_new->val = ipv;
        el_new->next = NULL;
